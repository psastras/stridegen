...
read -r -d '' __usage <<-'EOF' || true # exits non-zero when EOF encountered
  -f --file    [arg] Swagger file to process. Required.
  -o --output  [arg] Output directory to emit documentation.
...
[ -z "${arg_o:-}" ]     && help      "Setting an output directory with -o or --output is required"
...
output_dir=`absolute_path "${arg_o}"`
swagger_file=`absolute_path "${arg_f}"`

info "Reading Swagger definition from : ${output_dir}"
info "Writing HTML documentation to   : ${swagger_file}"

(cd /usr/local/lib/node_modules/stridegen; OUTPUT_DIR="${output_dir}" SWAGGER_FILE="${swagger_file}" npm run build)